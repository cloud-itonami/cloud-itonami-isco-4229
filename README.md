...
- `src/client_information/store.kotoba` — `Store` protocol +
  `MemStore`: registered clients, committed records, an append-only
  audit ledger.
- `src/client_information/advisor.kotoba` — `Advisor` protocol;
...
- `src/client_information/governor.kotoba` —
...
- `src/client_information/actor.kotoba` — `build-graph`, `run-request!`,
