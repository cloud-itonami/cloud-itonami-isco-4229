(ns client-information.actor
  "ClientInformationActor — the ISCO-08 4229 independent client-
  information-services actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  information operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the ClientInformationAdvisor can never
  directly commit a record or dispatch a robot action the
  ClientInformationGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [client-information.advisor :as advisor]
            [client-information.governor :as governor]
            [client-information.store :as store]))

(defn build-graph
  "Build a compiled ClientInformationActor graph. `store` implements
  `client-information.store/Store`. `advisor` implements
  `client-information.advisor/Advisor` (defaults to `mock-advisor`).
